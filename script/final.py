...
def process_file(input_file_path, cleaned_file_path, uszips1_df):
    ...
    try:
        ...
        df = df[['State', 'County', 'City'] + [col for col in df.columns if col not in ['State', 'County', 'City']]]
...
def main():
    ...
    try:
        new_data_df = new_data_df[['State', 'County', 'City'] + [col for col in new_data_df.columns if col not in ['State', 'County', 'City']]]
